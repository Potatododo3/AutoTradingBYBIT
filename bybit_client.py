    ...
    async def _request(
        ...
        for attempt in range(1, API_RETRY_ATTEMPTS + 1):
            try:
                ...
                if ret_code != 0:
                    # 34040 = "not modified" — trading-stop called with same SL/TP value.
                    # 110043 = "leverage not modified" — set-leverage called with same value.
                    # Both are no-op successes, not real errors.
                    if ret_code in (34040, 110043):
                        logger.debug(f"Bybit retCode {ret_code} (not modified) — treating as success")
